...
{
public:
    CLoadingScreenTextureRegenerator(unsigned char* pData, int width, int height)
        : m_pImageData(pData), m_nWidth(width), m_nHeight(height)
    {
    }
                ...
                for (int z = 0; z < nDepth; ++z)
                {
                    for (int iMip = 0; iMip < pVTFTexture->MipCount(); ++iMip)
                    {
                        ...
                        if (iMip == 0 && m_pImageData)
                            V_memcpy(pData, m_pImageData, nMipWidth * nMipHeight * 4);
                        else
